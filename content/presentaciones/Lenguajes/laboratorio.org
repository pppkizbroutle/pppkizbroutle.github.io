...
data Alumno = Alumno { nombre :: String
    ...
    #+begin_src haskell
recordAlumno (Alumno nombre noCuenta calif asistencias) = ...
    #+end_src
    ...
    #+begin_src haskell
califFinal (Alumno _ _ calif asistencias) = calif + fromIntegral asistencias / 10
    #+end_src
    ...
    #+begin_src haskell
califFinal1 Alumno{calif=c,asistencias=a} = c + fromIntegral a / 10
    #+end_src
    ...
    #+begin_src haskell
data Escuela = Estudiante { nombre :: String
                          , noCuenta :: String
                          , calif :: Double
                          , asistencias :: Int}
    ...
    #+begin_src haskell
describe Estudiante{} = "Es un alumno"
...
* Clases
  ...
  ¿Qué tipo tiene el operador de la suma en /Haskell/?
...
(+) :: Num a => a -> a -> a
  ...
  =Num a= hace referencia a que =a=, sea cual sea su tipo debe ser un
  número, posteriromente nos dice que recibe dos cosas de tipo =a= y nos
  regresa algo de tipo =a=.
  ...
  De la misma forma podemos hacerlo en /Haskell/ por medio de clases, en
  donde sólo indicaremos las firmas de las funciones o variables que
  la conforman.

  En el preludio de /Haskell/, =Num= se define como sigue:
...
class Num a where
  ...
  ¿Qué tipos de dato pertenecen a la clase =Num=?

  Con el comando =:i= haskell les puede mostrar información general
...
instance Num Word -- Defined in ‘GHC.Num’
  ...
  dato =Int=, =Double=, =Float=, =Integer= y =Word= son instancias de la clase
  =Num=, y por tanto podemos utilizar cualquier función definida en esta
  clase para estos tipos de dato.

  ¿Y cómo hacemos que un tipo de dato sea =Num=?
...
{-# MINIMAL (+), (*), abs, signum, fromInteger, (negate | (-)) #-}
  ...
  predefinida en /Haskell/ nos encontraremos con esta instrucción del
  compilador.
  ...
  - signo de un número (signum)

  - Conversión de entero al tipo de dato en cuestión (fromInteger)

  - Negación de un número (negate) o la resta (-) (Es decir, basta con que definamos alguna de las dos)
...
<interactive>:53:1: error:
  ...
  Aún no le hemos especificado a /haskell/ cómo se deben imprimir los
  números complejos.
...
data Complex = Complex Double Double deriving Show
  ...
  Con esto le indicamos a /haskell/ que queremos imprimir los números
...
Complex (-1.0) 0.0
  ...
  Bueno =Show= también es una clase
...
class Show a where
  ...
  =show= se usa para indicar directamente cómo queremos que se imprima.

  =showsPrec= es si en nuestros constructores queremos que se impriman
  con operadores indicando la precedencia que tiene cada uno de ellos.

  Para éste curso nos basta con implementar =show=.
...
* Práctica 1
  ...
  [[./practica1/practica1.org][Práctica 1]]

* Happy

  Es un generador de analizadores sintácticos en /Haskell/.

  Si tienen /cabal/, pueden instalarlo con la siguiente orden.

  #+begin_src bash
$ cabal install happy
  #+end_src

  La extensión que corresponde a los archivos de /happy/ comúnmente es
  =.y=

  Queremos implementar el siguiente lenguaje:

  $E\to \texttt{let <var> = }E\texttt{ in }E$
  ...
  Primero, necesitaremos un /lexer/.
...
lexer ('=':xs) = TokenAssign : lexer xs
lexer ('i':'n':xs) = TokenIn : lexer xs
...
lexer ('&':'&':xs) = TokenAnd : lexer xs
lexer ('|':'|': xs) = TokenOr : lexer xs
lexer ('(':xs) = TokenParOp : lexer xs
lexer (')':xs) = TokenParCl : lexer xs
lexer xs
  | not (null spaces) = lexer rest1
  | not (null digits) = TokenNum (read digits) : lexer rest2
  | not (null alpha) = TokenVar chars : lexer rest3
  | otherwise = error "Lexer error"
  where
    (spaces, rest1) = span isSpace xs
    (digits, rest2) = span isDigit xs
    (alpha, rest3) = span isAlpha xs
  #+end_src
